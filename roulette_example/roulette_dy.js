let ctntHeight = 128;  // use this to control disatnce of spin and stopping
...
(function($) {
	var Roulette = function(options) {
		...
		var slowDownSetup = function() {
			...
			p.maxDistance += ctntHeight - p.topPosition % ctntHeight;
			if (p.stopImageNumber != null) {
				// p.maxDistance += (p.totalHeight - (p.maxDistance % p.totalHeight) + (p.stopImageNumber * p.imageHeight))
				// 		% p.totalHeight;
				p.maxDistance += (p.totalHeight - (p.maxDistance % p.totalHeight) + (p.stopImageNumber * ctntHeight))
						% p.totalHeight;
			}
		}
